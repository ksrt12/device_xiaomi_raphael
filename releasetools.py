...
def AddTrustZoneAssertion(info, input_zip):
    ...
    if m:
        ...
        if len(versions) and '*' not in versions:
            cmd = 'assert(raphael.verify_trustzone(' + ','.join(['"%s"' % tz for tz in versions]) + \
                ') == "1" || abort("ERROR: This package requires firmware from an Android 10 based MIUI build. Please upgrade firmware and retry!"););'
...
def AddVendorAssertion(info, input_zip):
    ...
    for variant in ('in', 'cn', 'eea'):
        variants.append(re.search(
            r'require\s+version-{}\s*=\s*(\S+)'.format(variant), android_info).group(1).split(','))
